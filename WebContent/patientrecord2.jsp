...
<td><%=p.getAddress()%></td>
<td><a href="staff-patient2.jsp?a_id=<%=p.getAccountId()%>&fname=<%=p.getFirstName()%>&lname=<%=p.getLastName()%>&email=<%=p.getEmail()%>&telephone=<%=p.getTelephone()%>&address=<%=p.getAddress()%>">Update</a></td>
