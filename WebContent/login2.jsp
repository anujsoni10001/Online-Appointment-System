...
<td><a href="payment.jsp?a_id=<%=v.getAppointmentId()%>&charges=<%=v.getChargesRM()%>&d_email=<%=v.getDemail()%>&date=<%=v.getDate()%>&time=<%=v.getTime()%>&f_name=<%=v.getFirstName()%>&l_name=<%=v.getLastName()%>&doctor_name=<%=v.getDoctorName()%>">Pay</a></td>
</tr>
<%									
} 
%>
